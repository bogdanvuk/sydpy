...
class Jesd32bpLLookupPacker(sydpy.Component):
    ...
    def __init__(self, name, frame_out, ch_samples, tSample = None, arch='tlm', jesd_params=dict(M=1, N=8, S=1, CS=0, CF=0, L=1, F=1, HD=0), **kwargs):
        ...
        self.cur_frame_in = self.c['frame0']
        self.cur_frame_out = self.c['frame1']
        ...
        self.inst(sydpy.Process, 'pack', self.pack, senslist=[self.c['din0'].c['clk'].e['posedge']])
        self.inst(sydpy.Process, 'dispatch', self.dispatch, senslist=[self.c['din0'].c['clk'].e['posedge']])
    ...
    def pack(self):
        ...
        if self.c['segments_32b_cnt'].read() == self.segments_32b_num - 1:
            ...
            if self.c['cur_frame_out'].read() == 0:
                self.c['frame1'] <<= frame
                self.c['cur_frame_out'] <<= 1
            else:
                self.c['frame0'] <<= frame
                self.c['cur_frame_out'] <<= 0
